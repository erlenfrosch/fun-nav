...
async def test_circular_route_returns_up_to_three_routes(client):
    respx.get(GH_URL).mock(return_value=httpx.Response(200, json=make_gh_path()))
...
async def test_circular_route_no_routes_in_time_window(client):
    gh_response = {"paths": []}
    respx.get(GH_URL).mock(return_value=httpx.Response(200, json=gh_response))
...
async def test_circular_route_graphhopper_unreachable(client):
    respx.get(GH_URL).mock(side_effect=httpx.ConnectError("refused"))
...
async def test_circular_route_graphhopper_error_status(client):
    respx.get(GH_URL).mock(return_value=httpx.Response(500, text="internal error"))
